...
I have my plan as to how to design Gui over the ram head as , but what is holding me back is all the technical details that I needed to learn in order to make the application run accordingly. I went through so much frustration in this process. The header files were giving me many problems as without resolving the issues the header files offered, I could not really move ahead even with my goals and planning done ahead. My plan number two is to try this whole application in Visual Studio as Eclipse IDE is temperamental as it is (as per what I experienced) and see if I can get the Gui working as what I have planned of. The picture below was/ is what my expectations are/ were.
...
I faced a lot of challenges while carrying out this project. The first one was having to download the software application with all required libraries and folders, and this was after the class was over and done with. Once the application was up and running, then I had to go through old tutorials and do a lot of reading to understand what was being done in my old assignments. After this, I had to study a lot about how to create a GUI over the screen that holds a 3d object already. I came to know there were many GUI libraries that one can use, but some of them that topped the list were Dear ImGui, FlatGui, GLUI etc. After this, I downloaded ImGui as it sounded like a fair deal to me.
The header files were quite complicated to understand in one attempt. In case of ImGui, I came to learn that the here the header files and some selected source files have to be together in the same folder for me to be able to use some of the Gui functions in code and avoid any ‘undefined references’. I understood the strategy to create Gui and all the required artifacts within the interface, but the preliminary actions that I was required to take before even starting to code Gui is what took me so much time. Now my IDE is reading out an error message saying that it cannot find -lglfw library, and it is there but with a different name glfw3, and I am lost as to how to link the right file and there are a couple of GLFW functions used in the code to create Gui. I read a lot about making Cmakelists.txt file to assign right files or create a duplicate library file etc. to move forward, but the problem is that we did not really learn to make Cmakelists file in that class, and I am not much confident to say that it will completely solve the lingering problem.

So what I learnt through the whole process is that the software design and development is not an easy process to talk about and especially when you are the only one in the design and development process setting goals and making decisions. I also learnt about the importance of all the preliminary steps that need to be carried out to get the software application working to your desire. It maybe because I lack professional experience and aimed for some bigger project goals that I faced some serious challenges, but I have scaled back now and am hoping to just create a simple functioning Gui for this project eventually and I am working on it still (while addressing my other responsibilities for this class as well) and would sincerely appreciate the guidance and feedback from my professor on how to move forward. 

UPDATES:

 
After being able to remove some 40 errors down to 20 and now to 1. The IDE is still looking for this header file lgflw, which I guess technically meant glfw. The file is there but with a different name glfw3 and I could not link that file in place of glfw as the application is asking for. 
The picture below gives a glimpse of what I was trying to do in Gui coding and as one can see, I was able to use the ImGui functions well without any problems after a long time, but as the application is not able to find the header file lglfw, I could not really run it to see for myself. Through my research, I learnt that Eclipse not being able to find -lgflw is a common problem that programmers go through and there were some complex solutions given as well where they were using cMakelists file or using some gcc command line operations in DOS operating system to mitigate the problem but this may not be our course objectives I believe(?). 
